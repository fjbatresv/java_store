...
angular.module('doorman.controllers')
    ...
    $scope.logoChange = function (element) {
        $scope.$apply(function (scope) {
            ...
            var uploadTask = storage.child('images/' + $scope.cliente.cui + '/clientes/' + $scope.logo.name).put($scope.logo);
    ...
    .controller('ClientesEditController', function ($scope, $routeParams, $location, $mdToast, $mdDialog) {
        ...
        $scope.logoChange = function (element) {
            $scope.$apply(function (scope) {
                $scope.logo = element.files[0];
                var uploadTask = storage.child('images/' + $scope.cliente.cui + '/' + $scope.logo.name).put($scope.logo);
                uploadTask.on('state_changed', function (snapshot) {
                    $scope.logoLoading = true;
                    $scope.$apply();
                }, function (error) {
